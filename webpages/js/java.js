...
function validate(id1,id2,id3,id4,id5)
{
	...
	{	
		return (passwdmatch(document.getElementById('pass').value) && verifyEmail(document.getElementById('mail').value) && checkImg() && verifyUsername(document.getElementById('user').value));
	}
}

function passwdmatch(Pass)
{
	var Exp = /^([A-Za-z0-9]+)$/;
        var Reg = new RegExp(Exp);
        if(Pass.length<5 || !Reg.test(Pass))
        {
                alert("Password should contan atleast 5 alphanumeric characters");
                return false;
        }
...
function verifyUsername(userName)
{
	...
	var Exp = /^([A-Za-z0-9]+)$/;
	var Reg = new RegExp(Exp);
	if(userName.length<5 || !Reg.test(userName))
	{
		alert("Username should contan atleast 5 alphanumeric characters");
		okay = false;
	}
	return okay;
}
